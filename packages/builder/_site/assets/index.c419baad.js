...
for (let propId of ["x7BmXv", "Iy5OBB", "Zz4apn", "wR2-lv", "BHLwBQ"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="trzs9V"]');
Promise.race([
  load()
]).then(async function() {
...
const props = {};
for (let propId of ["i8Zpw_"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="jgt_8J"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
