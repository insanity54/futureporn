...
for (let propId of ["xlh4PD", "UHUdkB", "I7HTzU", "HvFhzq", "tdVit4"]) {
  const { name, value } = propsById[propId];
  props$3[name] = value;
}
const target$3 = document.querySelector('slinkity-root[data-id="JoCO_Q"]');
Promise.race([
  load()
]).then(async function() {
...
const props$2 = {};
for (let propId of ["_GWjbP", "iJ8Vgx"]) {
  const { name, value } = propsById[propId];
  props$2[name] = value;
}
const target$2 = document.querySelector('slinkity-root[data-id="RmZjWh"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props$1 = {};
for (let propId of ["riQYVf"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="My10LK"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["Ywgqx-"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="cwv5re"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
