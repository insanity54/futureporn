...
const props$2 = {};
const target$2 = document.querySelector('slinkity-root[data-id="lLAPAo"]');
Promise.race([
  load()
]).then(async function() {
...
const props$1 = {};
for (let propId of ["L0Nec1"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="7MpE3x"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["CqYV0d"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="bO61cz"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
