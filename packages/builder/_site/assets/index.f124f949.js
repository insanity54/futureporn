...
const props$2 = {};
const target$2 = document.querySelector('slinkity-root[data-id="7aqKhZ"]');
Promise.race([
  load()
]).then(async function() {
...
const props$1 = {};
for (let propId of ["TcaR6c"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="N_JzUi"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["NQTcTv"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="N0GMfc"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
