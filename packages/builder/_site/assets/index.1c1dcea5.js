...
const props$2 = {};
const target$2 = document.querySelector('slinkity-root[data-id="8mutWv"]');
Promise.race([
  load()
]).then(async function() {
...
const props$1 = {};
for (let propId of ["1zyXX2"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="Kdl5y6"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["U0eWSV"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="9k1K6o"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
