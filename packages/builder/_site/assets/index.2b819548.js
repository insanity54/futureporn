...
const props$3 = {};
for (let propId of ["40WV1S", "_X-sMR", "mnD7j5", "uQe9jH", "d2HSSe"]) {
  const { name, value } = propsById[propId];
  props$3[name] = value;
}
const target$3 = document.querySelector('slinkity-root[data-id="Fnn1sp"]');
Promise.race([
  load()
]).then(async function() {
...
const props$2 = {};
for (let propId of ["beoDxa", "ZggSaC"]) {
  const { name, value } = propsById[propId];
  props$2[name] = value;
}
const target$2 = document.querySelector('slinkity-root[data-id="0EVeDl"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props$1 = {};
for (let propId of ["WO2IXI"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="PO3Lm0"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["hJQ8qw"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="sJwT7Y"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
