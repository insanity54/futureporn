...
for (let propId of ["x8JJwe", "WnmQHp", "m3HTKd", "SWEEF_", "GWIgIS"]) {
  const { name, value } = propsById[propId];
  props$3[name] = value;
}
const target$3 = document.querySelector('slinkity-root[data-id="bSCmPs"]');
Promise.race([
  load()
]).then(async function() {
...
const props$2 = {};
for (let propId of ["pH91La", "JerR8V"]) {
  const { name, value } = propsById[propId];
  props$2[name] = value;
}
const target$2 = document.querySelector('slinkity-root[data-id="YUDhCY"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props$1 = {};
for (let propId of ["ykMeDs"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="j0UwOD"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["pqk85X"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="kzPG04"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
