...
for (let propId of ["GD_HuD", "fQd99q", "3WQ20H", "zQeaZk", "XqUDOc"]) {
  const { name, value } = propsById[propId];
  props$3[name] = value;
}
const target$3 = document.querySelector('slinkity-root[data-id="FYBvE5"]');
Promise.race([
  load()
]).then(async function() {
...
const props$2 = {};
for (let propId of ["PYgFbC", "jEPC5c"]) {
  const { name, value } = propsById[propId];
  props$2[name] = value;
}
const target$2 = document.querySelector('slinkity-root[data-id="hCuOPh"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props$1 = {};
for (let propId of ["eDZcNC"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="eUSW2z"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["VR1TAK"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="gLiDyg"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
