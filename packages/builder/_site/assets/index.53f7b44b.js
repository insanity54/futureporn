...
const props$2 = {};
const target$2 = document.querySelector('slinkity-root[data-id="sjWlE8"]');
Promise.race([
  load()
]).then(async function() {
...
const props$1 = {};
for (let propId of ["Ew-LeG"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="pdFJTM"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["XjBm6f"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="Vwas2W"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
