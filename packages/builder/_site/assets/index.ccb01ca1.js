...
for (let propId of ["vBz_hC", "vLlPH-", "zCsFBN", "gBbhd-", "1NN8H3"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="HpPaoX"]');
Promise.race([
  load()
]).then(async function() {
...
const props = {};
for (let propId of ["axYaod", "I6HElm"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="UddcUW"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
