...
for (let propId of ["F0wjJf", "EBiQ-1", "e3gl0i", "Dz9Azb", "WBkvu7"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="tr9v0_"]');
Promise.race([
  load()
]).then(async function() {
...
const props = {};
for (let propId of ["fX40D4"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="KSCepo"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
