...
const props$2 = {};
const target$2 = document.querySelector('slinkity-root[data-id="VdW_eD"]');
Promise.race([
  load()
]).then(async function() {
...
const props$1 = {};
for (let propId of ["uE19e4"]) {
  const { name, value } = propsById[propId];
  props$1[name] = value;
}
const target$1 = document.querySelector('slinkity-root[data-id="ZkizWX"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
...
const props = {};
for (let propId of ["X8J2D1"]) {
  const { name, value } = propsById[propId];
  props[name] = value;
}
const target = document.querySelector('slinkity-root[data-id="vmLsgV"]');
Promise.race([
  load()
]).then(async function() {
  const [{ default: Component }, { default: renderer }] = await Promise.all([
